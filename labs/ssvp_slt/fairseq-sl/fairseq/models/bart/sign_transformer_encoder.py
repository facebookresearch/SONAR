...
def module_name_fordropout(module_name: str) -> str:
    if module_name == "SignTransformerEncoderBase":
        return "SignTransformerEncoder"
    else:
        return module_name


class SignTransformerEncoderBase(FairseqEncoder):
    """
    ...
    def __init__(self, cfg, dictionary, embed_tokens, return_fc=False):
        ...
        self.feature_proj = nn.Linear(1024, 1024)
    ...
    def forward_scriptable(self, src_tokens, src_lengths, return_all_hiddens, token_embeddings):
        """
        Args:
            src_tokens (LongTensor): tokens in the source language of shape
                `(batch, src_len)`
        ...
        encoder_padding_mask = src_tokens
        ...
        x = encoder_embedding = self.embed_scale * self.feature_proj(token_embeddings)
        if self.embed_positions is not None:
            x = encoder_embedding + self.embed_positions(encoder_padding_mask)
        if self.layernorm_embedding is not None:
            x = self.layernorm_embedding(x)
        x = self.dropout_module(x)
        if self.quant_noise is not None:
        ...
        # print(f"{x.shape = }")
        # print(f"{encoder_padding_mask.shape = }")
        ...
        inv_encoder_padding_mask = ~encoder_padding_mask
        src_lengths = inv_encoder_padding_mask.long().sum(dim=1, dtype=torch.int32).reshape(-1, 1).contiguous()
        # print(f"{src_lengths = }")
        # print(f"{[e.shape for e in encoder_states] = }")
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        ...
        if "encoder.feature_proj.weight" not in state_dict:
            feature_proj = nn.Linear(1024, 1024)
            feature_proj.apply(init_bert_params)
            state_dict["encoder.feature_proj.weight"] = feature_proj.weight
            state_dict["encoder.feature_proj.bias"] = feature_proj.bias
        return state_dict


class SignTransformerEncoder(SignTransformerEncoderBase):
